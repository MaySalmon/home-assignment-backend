...
export class EmployeesService {
  ...
  async updateStatus(id: string, updateStatusDto: UpdateStatusDto): Promise<Employee> {
    const employee = await this.findOne(id);
  ...
  async remove(id: string): Promise<void> {
    ...
    if (employee.avatarUrl) {
      const filePath = path.join(process.cwd(), 'uploads', path.basename(employee.avatarUrl));
  ...
  async updateAvatar(id: string, filename: string, baseUrl: string): Promise<Employee> {
    const employee = await this.findOne(id);
    if (employee.avatarUrl) {
      const oldPath = path.join(process.cwd(), 'uploads', path.basename(employee.avatarUrl));
